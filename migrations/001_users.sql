...
CREATE VIEW detailed_user_profiles AS
SELECT
  u.id,
  u.username,
  u.status,
  ...
  CASE u.status
    ...
    WHEN 'banned'    THEN u.banned_reason
    WHEN 'rejected'  THEN u.rejected_reason
  END AS status_reason,

  CASE u.status
    WHEN 'requested' THEN u.created_at
    WHEN 'approved'  THEN u.approved_at
    WHEN 'banned'    THEN u.banned_at
    WHEN 'rejected'  THEN u.rejected_at
  END AS status_at,

  CASE u.status
    WHEN 'requested' THEN u.created_by
    WHEN 'approved'  THEN approver.username
    WHEN 'banned'    THEN banner.username
    WHEN 'rejected'  THEN rejector.username
  END AS status_by,
