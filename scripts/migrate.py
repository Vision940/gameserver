...
def get_applied_migration(cur, namespace, title):
    cur.execute(
        """
        SELECT checksum
...
def record_migration(cur, namespace, title, checksum):
    cur.execute(
        """
...
def apply_migration_file(cur, namespace, path):
    ...
    applied = get_applied_migration(cur, namespace, title)
    ...
    record_migration(cur, namespace, title, checksum)
    return True


def apply_migration_dir(cur, namespace, directory):
    applied_count = 0

    for migration in migration_files(directory):
        if apply_migration_file(cur, namespace, migration):
            applied_count += 1

    return applied_count


def apply_game_migrations(cur):
    applied_count = 0
    ...
    for game in GAME_LIST:
        ...
        applied_count += apply_migration_dir(cur, f"game:{game.source_name}", migrations_dir)
...
def migrate():
    ...
    with db.db_cursor() as cur:
        ...
        core_count = apply_migration_dir(cur, "core", CORE_MIGRATIONS_DIR)
        game_count = apply_game_migrations(cur)
